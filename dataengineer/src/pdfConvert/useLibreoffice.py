...
def convert_folder_pdf_to_html(input_folder, output_folder):
    ...
    for pdf_file in pdf_files:
        ...
        try:
            ...
            # Appeler la commande de conversion de LibreOffice
            command = f"libreoffice --invisible --convert-to html --outdir {output_folder} {pdf_path}"
            # command = f"libreoffice --invisible --headless --convert-to docx:\"MS Word 2007 XML\" --outdir {output_folder} {pdf_path}"
            subprocess.run(command, shell=True)
            # Encoder les images en base64 dans le fichier HTML
            # encode_images_to_base64(html_path)
            print(f"Conversion de '{pdf_path}' vers '{html_path}' terminée.")
...
if __name__ == "__main__":
    ...
    output_docx_path = os.path.join(cwd, 'storage/docs')
    convert_folder_pdf_to_html(input_folder_path, output_html_path)
    # convert_folder_pdf_to_word_doc(input_folder_path, output_docx_path)
